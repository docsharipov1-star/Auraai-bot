...
SYSTEM_PROMPTS = {
    ...
    "relations": (
        ...
        "ЯЗЫК: отвечай на том же языке что пишет пользователь — русский, английский, узбекский, казахский, украинский, любой другой.\n\n"
        ...
        "— Несколько вопросов подряд — МАКСИМУМ ОДИН вопрос за ответ\n\n"
        ...
        "2. Сделай наблюдение терапевта или задай ОДИН точный вопрос — и остановись\n"
        "3. Когда контекст ясен — помоги увидеть паттерн или предложи инструмент\n\n"

        "СТИЛЬ: живой, без осуждения, иногда прямой. 3–5 предложений. "
    ...
    "health_psy": (
        ...
        "ЯЗЫК: отвечай на языке пользователя — русский, английский, узбекский, казахский, украинский, любой другой.\n\n"
        "МАКСИМУМ ОДИН вопрос за ответ. Чаще говори — не спрашивай.\n\n"
        "СТИЛЬ: заботливый, точный, без запугивания. 3–5 предложений. "
    ...
    "psy": (
        ...
        "Русский, английский, узбекский, казахский, таджикский, украинский, немецкий, испанский и любой другой — "
        "ты понимаешь и отвечаешь на языке человека. Никогда не переключай язык без просьбы.\n\n"

        "ГЛАВНОЕ ПРАВИЛО — ОДИН ВОПРОС МАКСИМУМ:\n"
        "❌ НИКОГДА не задавай 2 и более вопросов в одном сообщении.\n"
        "❌ НЕ заканчивай каждый ответ вопросом — это шаблон бота, а не терапевта.\n"
        "✅ Чаще всего скажи что-то точное про человека — и замолчи. Дай пространство.\n"
        "✅ Вопрос только если без него невозможно двигаться. И только ОДИН.\n\n"

        "ЗАПРЕЩЕНО АБСОЛЮТНО:\n"
        "— Советы: «напиши прости», «поговори с ней», «отдохни», «займись собой»\n"
        ...
        "Он слышит боль, злость, сожаление — и отражает ТОЧНО. "
        "«Ты послал её... и сейчас внутри что — облегчение? злость? пустота?» "
        "Потом молчит. Ждёт. Не задаёт следующий вопрос.\n\n"

        "ТВОЙ АЛГОРИТМ:\n"
        "1. Услышь что за словами — эмоция, боль, потребность, страх\n"
        "2. Отрази это ТОЧНО и метко — одной фразой\n"
        "3. Либо сделай наблюдение терапевта: «похоже за этой злостью стоит что-то большее»\n"
        "4. Либо если совсем неясно — ОДИН точный вопрос\n"
        "5. Остановись. Не добавляй ничего лишнего.\n\n"

        "СТИЛЬ:\n"
        "— Живой, тёплый — как умный друг который умеет слушать\n"
        "— 2–4 предложения обычно. Развёрнуто только при технике\n"
        "— Ссылайся на то, что человек говорил раньше\n\n"

        "БЕЗОПАСНОСТЬ:\n"
        "— Без диагнозов и лекарств\n"
        "— Кризис: поддержи + 8-800-2000-122 (РФ) / 0800-33-11 (UA) / 150 (KZ)"
    ),
}
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        migrations = [
            ...
            "ALTER TABLE users ADD COLUMN psy_free_used INTEGER DEFAULT 0",
            "ALTER TABLE users ADD COLUMN psy_sub_expires TEXT",
        ]
...
PSY_FREE_LIMIT = 10  # Бесплатных сессий с психологом
PSY_SUB_STARS  = 400  # Цена подписки в Telegram Stars (~490₽/мес)
PSY_SUB_DAYS   = 30

async def check_psy_access(uid: int) -> tuple[bool, int]:
    """Возвращает (разрешён доступ, сколько бесплатных осталось)."""
    row = await db_get("SELECT psy_free_used, psy_sub_expires, plan FROM users WHERE id=?", (uid,))
    if not row:
        return True, PSY_FREE_LIMIT
    # Premium план = безлимит психолог
    if row["plan"] == "premium":
        return True, 999
    # Активная psy-подписка
    if row["psy_sub_expires"]:
        from datetime import datetime
        try:
            exp = datetime.fromisoformat(row["psy_sub_expires"])
            if exp > datetime.now():
                return True, 999
        except Exception:
            pass
    used = row["psy_free_used"] or 0
    remaining = max(0, PSY_FREE_LIMIT - used)
    return remaining > 0, remaining

async def increment_psy_used(uid: int):
    await db_run(
        "UPDATE users SET psy_free_used = COALESCE(psy_free_used,0) + 1 WHERE id=?", (uid,)
    )

async def activate_psy_sub(uid: int):
    from datetime import datetime, timedelta
    expires = (datetime.now() + timedelta(days=PSY_SUB_DAYS)).isoformat()
    await db_run("UPDATE users SET psy_sub_expires=? WHERE id=?", (expires, uid))

def psy_paywall_kb():
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(
        text=f"💳 Подписка психолог — {PSY_SUB_STARS} ⭐",
        callback_data="buy_psy_sub"
    ))
    return b.as_markup()
...
PSY_TOOLS = {
    ...
    "🔁 Разобрать мысль": (
        "Человек хочет выгрузить накопленные мысли и разобраться в них. "
        "Это могут быть мысли о чём угодно — работа, отношения, тревоги, обиды, планы, страхи, путаница в голове. "
        "Не задавай вопросы — дай пространство. Скажи тепло: «Расскажи всё что накопилось — "
        "я здесь, никуда не тороплюсь. Выгружай как есть.» "
        "Когда человек выскажется, помоги разобрать мысли по КПТ: найди главную тему, "
        "выяви когнитивные искажения если есть, предложи другой взгляд. "
        "Работай с тем что реально принесёт человек, не предполагай заранее."
    ),
    "🌍 Заземлиться": (
...
async def process_text(message: Message, state: FSMContext):
    ...
    if tool_id in PSY_TOOLS_ALL:
        ...
        if user_text in PSY_TOOLS:
            ...
            # Инструменты (Разобрать мысль, Дневник) работают с любой темой —
            # используем общий psy-промпт а не специфический (отношения/здоровье)
            tool_id = "psy"
    ...
    # Freemium: проверяем доступ к психологу
    PSY_TOOLS_IDS2 = ("psy", "relations", "health_psy", "coach")
    if tool_id in PSY_TOOLS_IDS2:
        has_access, remaining = await check_psy_access(message.from_user.id)
        if not has_access:
            await message.answer(
                "🔒 *Твои 10 бесплатных сессий с психологом завершены.*\n\n"
                "Для продолжения оформи подписку — это даёт безлимитный доступ к психологу, "
                "разбору мыслей, поддержке по отношениям и здоровью на 30 дней.\n\n"
                f"💳 *{PSY_SUB_STARS} Telegram Stars* (~490₽) — 30 дней",
                parse_mode="Markdown",
                reply_markup=psy_paywall_kb()
            )
            await state.set_state(State_.waiting_text)
            await state.update_data(tool=tool_id, model=model_id, cost=cost)
            return
        if remaining <= 3 and remaining > 0:
            await message.answer(
                f"💡 У тебя осталось *{remaining}* бесплатных сессий с психологом.",
                parse_mode="Markdown"
            )
    ...
    try:
        ...
        await log_request(message.from_user.id, tool_id, model_id, cost)
        if tool_id in ("psy", "relations", "health_psy", "coach"):
            await increment_psy_used(message.from_user.id)
...
@router.callback_query(F.data == "buy_psy_sub")
async def cb_buy_psy_sub(callback: CallbackQuery):
    await callback.answer()
    await callback.message.answer_invoice(
        title="Подписка на AI-психолога",
        description=(
            "30 дней безлимитного доступа к AI-психологу Аура:\n"
            "• Психолог, Отношения, Здоровье\n"
            "• Разбор мыслей без ограничений\n"
            "• Поддержка 24/7"
        ),
        payload="psy_sub",
        currency="XTR",
        prices=[LabeledPrice(label="Подписка 30 дней", amount=PSY_SUB_STARS)],
    )
...
async def on_payment(message: Message):
    ...
    elif payload == "psy_sub":
        item_name = "AuraAI — Подписка психолог 30 дней"
        await activate_psy_sub(uid)
        await message.answer(
            "✅ *Подписка на AI-психолога активирована на 30 дней!*\n\n"
            "Теперь у тебя безлимитный доступ к:\n"
            "🧠 Психолог · 💕 Отношения · 🌿 Здоровье · 🔁 Разбор мыслей\n\n"
            "Аура ждёт тебя 💛",
